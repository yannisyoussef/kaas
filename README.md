...
| Runner bootstrap | IMPLEMENTED + VALIDATED | Drives the full execution lifecycle over the internal API and runs no user-supplied content |
...
| Sandbox security attestation | **IMPLEMENTED + VALIDATED** | Signed by the gate that made the observations and verified against a deployment-pinned Ed25519 key (ADR-027). An operator transports the artifact and no longer authors its verdicts; the control plane holds verification authority and structurally cannot sign. `v4` required (ADR-028 added the signed sandbox runtime); v3 and unsigned v2 refused with no fallback |
...
It deliberately does not implement SSE, secret values or storage or redemption against a real provider, object storage, Karate parsing or execution, network enforcement beyond deny-all, artifacts, quality-gate execution, worker heartbeating during execution, or full OpenTelemetry infrastructure. A claim records who owns an infrastructure attempt; an authorization records that this specific assignment may execute; and a synthetic execution now acts on both — running a workload the platform wrote, so that the lifecycle around an execution can be proven correct before any tenant content is admitted to it. No FeatureRevision source enters a sandbox, and the runner's own types carry nothing that could hold it. That boundary is still the point.
