...
**Status: PARTIALLY IMPLEMENTED.** The pure transition oracle, exact `none → CREATED` persistence, the atomic `CREATED → QUEUED` scheduling transition, and the three transitions that end a run no worker has taken — `CREATED → COMPLETED` and `QUEUED → COMPLETED` by cancellation, and `QUEUED → COMPLETED` by queue deadline — are implemented. `QUEUED → CLAIMED` and everything past it, including every claim/lease/messaging handler and every deadline other than the queue deadline, remain proposed. See [the early terminal lifecycle slice](early-terminal-lifecycle-slice.md).
...
| `PROCESSING_RESULTS`,`COMPLETED` | `REQUEST_CANCEL` → unchanged | API | **IMPLEMENTED for `COMPLETED`:** current state re-read after the lock predicate fails | No lifecycle mutation | none | Idempotent by state: a run already cancelled replays its terminal representation as 200; one that ended for any other reason is a conflict | Terminal or point-of-no-return | Returns 409 `RUN_ALREADY_TERMINAL` for a run that completed for another reason, and 409 `RUN_NOT_CANCELLABLE` for a phase early cancellation cannot end; never discards accepted evidence |
