...
**Status: PARTIALLY IMPLEMENTED.** Implemented: the pure transition oracle, exact `none → CREATED` persistence, `CREATED → QUEUED` scheduling, the three transitions that end a run no worker has taken (`CREATED → COMPLETED` and `QUEUED → COMPLETED` by cancellation, `QUEUED → COMPLETED` by queue deadline), `QUEUED → CLAIMED` by the dispatch consumer, and `CLAIMED → STOPPING → COMPLETED` by cancellation or lease loss — with the assignment epoch as a fencing token, a server-controlled lease, and heartbeats. `CLAIMED → PROVISIONING → RUNNING → COLLECTING_RESULTS → PROCESSING_RESULTS → COMPLETED` is now implemented and driven by a real worker, along with result acceptance, per-phase deadlines, and a reconciler that stops and fences an overdue run — see [the synthetic execution lifecycle](synthetic-execution-lifecycle.md) and [ADR-024](../adr/024-synthetic-execution-lifecycle.md). What executes is a platform-owned synthetic workload rather than a test engine. Artifact handling remains proposed, as does worker heartbeating during execution. See [the consumer/claim/lease slice](consumer-claim-lease-slice.md) and [the early terminal lifecycle slice](early-terminal-lifecycle-slice.md).
