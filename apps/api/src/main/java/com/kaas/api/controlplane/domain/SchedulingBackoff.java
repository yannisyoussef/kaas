...
/**
 * Bounded, durable retry delay for the scheduler. Distinct from broker publication retry, execution retry, and
 * Karate scenario retry: this one governs only how soon a CREATED run may be offered to the scheduler again.
 *
 * <p>A scheduling failure never terminalizes the run. Infrastructure being briefly unhealthy is not a verdict on
 * a test run, so the delay grows and is eventually capped, and only a bounded number of failures moves the run
 * into quarantine for an operator to look at.
 *
