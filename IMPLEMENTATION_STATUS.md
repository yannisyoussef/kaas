...
Do not add Karate, a real secret provider, egress beyond deny-all, or any path that reads FeatureRevision source inside a sandbox until the prerequisites named in ADR-022, ADR-023, and ADR-025 are met and the gate passes on the deployment's own runtime. Do not run anything longer than a lease until a worker heartbeats during execution: the lease currently has to outlive the whole run, which the 30-minute execution budget exceeds.
