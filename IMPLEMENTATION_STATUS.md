...
See `SIGNED_SECURITY_ATTESTATION_SLICE_REPORT.md` for the signed-attestation slice: the unsigned weakness, the Ed25519 and pinned-key decisions, the canonical signed payload and its cross-checked vectors, the producer, the verifier, runtime subject and generation semantics, the v2 rejection, the mutation ledger, and the residual risks.
