...
| Capability | Status |
|---|---|
| Karate execution | **ABSENT / DISABLED** — no Karate runtime exists in any dependency graph, and no `.feature` file is parsed by anything. Version 2.1.2 was evaluated as an artifact; no dependency was added |
...
| Tenant-controlled code execution | **APPROVED FOR ONE SECRET-FREE SLICE, NOT YET IMPLEMENTED** — ADR-032; the runtime-identity gap is closed and `nodev` is accepted with tested compensating controls. Nothing executes tenant content today |
...
- Karate dependencies, a production secret provider, secret capability issuance, artifact retention, or execution of tenant content. Tenant source is now *delivered* under ADR-030 and remains inert: it is hashed and compared, never run. A command is delivered and executed under ADR-024, but what it executes is a platform-owned synthetic workload (`KAAS_SYNTHETIC_V1`): no FeatureRevision source enters a sandbox, no secret is resolved, and the declared engine is `SYNTHETIC` rather than `KARATE` — the runner refuses any engine it does not have.
