...
- **Inert tenant source delivery (ADR-030): IMPLEMENTED + VALIDATED, WITH ONE MEASURED GAP.** A worker redeems an assignment-scoped SourceCapability, verifies the delivered bundle against the command's own feature list and digests, stages it as `0444` regular files, mounts it read-only, and the platform's verifier re-hashes the bytes **from inside the sandbox**. Tenant bytes are stored, transported, mounted, read, hashed and compared, and nothing else: no parser, no interpreter, no shell, no launcher argument, no container setting.
  - **The gap, stated rather than buried: `noexec` is not carried onto the source mount under the mediating runtime.** Measured on both runtimes — a `local`-driver bind arrives as ext4 with `ro,nosuid,nodev,noexec` under the baseline and as a 9p gofer mount with `ro` alone under gVisor, where an executable file in it ran. What refuses execution instead is that the bundle format cannot express a mode and the materialiser writes no executable bit. The requirement was not downgraded; **tenant code execution remains NOT APPROVED**, and this gap is a prerequisite for that decision alongside the KAAS-15 runtime-pin attestation gap. See `TENANT_SOURCE_DELIVERY_SLICE_REPORT.md` and [the delivery document](docs/security/tenant-source-delivery.md).
  ...
  - Staging is `AutoCloseable` and owned by a try-with-resources, so tenant bytes do not outlive the execution that fetched them on any path including failure; a reconciler on the execution path reclaims what a crashed host left, matching only the platform's directory prefix and only past a grace period.
...
| FeatureRevision execution | **ABSENT** — source now *enters* a sandbox as inert data and is hashed; nothing executes, parses or interprets it |
...
| Tenant-controlled code execution | **NOT APPROVED** — ADR-022 stays open; blocked additionally on the mount `noexec` gap and the runtime-pin attestation gap |
