...
This document describes repository reality after the tenant execution readiness adjudication (KAAS-20). Product vision and execution architecture do not imply runtime capability: the repository executes a platform-owned synthetic workload. Tenant-authored bytes now reach a sandbox for the first time, as **data** — they are mounted read-only, hashed and compared. **No tenant content is executed, parsed or interpreted by anything.**
...
- **Mediated hostile-content boundary: IMPLEMENTED + VALIDATED FOR FUTURE INERT TENANT SOURCE DELIVERY.** ADR-022's runtime prerequisite is satisfied for the mediated runtime, which permits a future slice to deliver *inert* tenant-authored source bytes into the sandbox to be hashed and inspected. **Tenant code execution is not approved.** Karate execution, FeatureRevision execution, tenant source delivery, a production secret provider and tenant secret injection all remain ABSENT. See `HOSTILE_CONTENT_READINESS_SLICE_REPORT.md` and [the readiness document](docs/security/hostile-content-readiness.md), which lists the residual risks accepted for non-executing bytes and the seven requirements binding on the delivery slice.
- **Inert tenant source delivery (ADR-030, superseded by ADR-031): IMPLEMENTED + VALIDATED AS INERT DATA.** A worker redeems an assignment-scoped SourceCapability, verifies the delivered bundle against the command's own feature list and digests, and hands it to the sandbox. Tenant bytes are stored, transported, written, read, hashed and compared, and nothing else: no parser, no interpreter, no shell, no launcher argument, no container setting.
  - The plaintext capability is bearer authority and lives only in the delivery envelope: never in the ExecutionCommand, its digest, PostgreSQL plaintext, the broker, the outbox or inbox, a container label, a filesystem name, a metric, a log line, an exception, or CI output.
- **Mediated source filesystem (ADR-031): IMPLEMENTED + VALIDATED, WITH ONE RUNTIME LIMITATION.** The source filesystem is a sandbox-private tmpfs that a platform-owned bootstrap populates over the container's standard input and then freezes read-only before dropping every capability. There is no host mount of tenant source anywhere, so there is no weaker second copy for a future engine to find, and no host staging directory at all.
  - **Source mount read-only: IMPLEMENTED + VALIDATED.** Reported, a write refused, and the consumer cannot remount it.
  - **Source mount noexec: IMPLEMENTED + VALIDATED.** This is the KAAS-18 blocker, closed. Proven against a file with mode `0555` that runs on a permissive filesystem in the same sandbox and is refused on this one.
  - **Source mount nosuid: REPORTED.** The runtime performs no setuid transition on any filesystem — measured, while the baseline runtime escalates in the identical test — so the flag has no behavioural red path here and none is claimed.
  - **Source mount nodev: NOT ENFORCED.** gVisor does not implement `MS_NODEV`. Standing in its place: a read-only filesystem, an empty bounding set holding no `CAP_MKNOD`, and a format that cannot express a device node. None of those is the flag, and it is recorded as a gap. See `MEDIATED_SOURCE_FILESYSTEM_BOUNDARY_SLICE_REPORT.md` and [the security document](docs/security/mediated-source-filesystem.md).
  - The construction phase holds `CAP_SYS_ADMIN` and three identity capabilities for the length of one populate-and-freeze. Every control is measured on the **consumer**, after the drop: all capability sets empty, uid 65534, remount refused. Without the capability the freeze fails and nothing becomes ready.
  - Source delivery is refused outright on a runtime that cannot enforce the boundary, before a container exists.
- **Tenant execution readiness (ADR-032): ADJUDICATED. Tenant code execution is APPROVED FOR ONE SECRET-FREE SLICE and remains ABSENT until that slice exists.**
  - **The execution model is decided: tenant Karate is arbitrary JVM code.** Karate 2.1.2 resolves `Java.type` to `Class.forName` on the context classloader with no allowlist, no class filter and no configuration gate — read from the artifact's bytecode, not from its documentation. A restricted DSL is not available without forking the engine, so containment is the boundary and the future engine image's classpath is a security control.
  - **Runtime implementation identity: IMPLEMENTED + VALIDATED (attestation v5).** Signed evidence now binds the sandbox runtime binary's version, SHA-256 and an opaque path identity, measured from the daemon's own registration with no operator-supplied value anywhere. Replacing the runtime binary makes every existing attestation stop authorizing execution, automatically. A v4 document is refused as a superseded schema, proven by a signed, trusted, refused vector.
  - **Hostile JVM containment: MEASURED.** Under the mediating runtime, with the posture a future engine inherits: every capability set empty, source filesystem write/chmod/exec/remount/`mknod` all refused, scratch space writable and non-executable so generated code cannot run, DNS and three socket destinations refused, no daemon socket, no credential in the environment, and Java threads bounded by the PID ceiling.
  - **`nodev` remains NOT ENFORCED and is now ACCEPTED for execution** with four separately tested compensating controls. Recorded as `NODEV NOT ENFORCED, COMPENSATING CONTROLS ACCEPTED FOR EXECUTION` — never as a pass — and the gate asserts `source_mount_nodev=false` so the fact stays visible in every green run.
  - **Karate is still ABSENT.** No dependency was added; version 2.1.2 was evaluated as a downloaded artifact. See `TENANT_EXECUTION_READINESS_SLICE_REPORT.md` and [the readiness document](docs/security/tenant-execution-readiness.md).
- **Secret-free Karate execution (ADR-033): IMPLEMENTED + VALIDATED. Karate 2.1.2 now executes real tenant features.** A bundle is authorized, verified, framed, delivered over stdin, written by the platform bootstrap, frozen read-only, and handed to a JVM that starts Karate. The platform reads back one key.
  - **It runs under the mediating runtime, and only there.** The engine executes on a filesystem the bootstrap closes behind itself, and ADR-031 measured that closing remount as refused under the baseline runtime (`bootstrap_failure=FREEZE`). So there is no baseline configuration in which the engine starts at all. The suite is therefore not part of `check`; its evidence comes from the mandatory `karate-execution-gate` job, which installs `runsc`. **A green local build proves nothing about it.**
  - **Karate lives in exactly one module.** `services/karate-engine` is the only module permitted to carry it, under either coordinate; the runner obtains the engine by resolving its *image build context*, never as a project dependency, so it launches an engine without linking one. Gradle guards and a runtime classloader test enforce this from both sides.
  - **The version pin is verified where the image is built.** `KarateEngineImage` refuses a context shipping the wrong version, no engine, or two — the last because a wildcard classpath would otherwise let directory iteration order choose the engine.
  - **A duplicated result key is refused, not resolved.** Tenant code can print the protocol line (measured, via `System.out.println` through Java interop). Forging turns the run into an infrastructure failure rather than a pass. A tenant can still influence its own test outcome; that is accepted and was accepted by ADR-032.
  - **Absent evidence is never a pass.** A killed run, a stdout flood, and `System.exit(0)` mid-suite all report `ABSENT`. The last leaves a container that exited **zero** having run no assertion.
  - **Source resolution is deliberately NOT confined, reversing ADR-032's expectation.** Measurement showed Karate applies no root confinement, and a wrapper over `read()` would not be a boundary while `Files.readAllBytes` is one `Java.type` away. The platform confines what there is to read: a secret-free digest-pinned image, **exactly three** environment variables, no host mount, and no secret. What it does confine is the set that gets *executed* — the manifest, never a scan.
  - **Zero secret bindings, refused before the provider check.** `ENGINE_REQUIRES_SECRET_FREE_RUN`, with the runner refusing independently. See `SECRET_FREE_KARATE_EXECUTION_SLICE_REPORT.md`, [the security document](docs/security/secret-free-karate-execution.md), and [the execution path](docs/architecture/karate-execution-path.md).
- **Continuous execution authority (ADR-029).** A worker maintains evidence that it still owns its assignment for the whole of every phase it owns. The heartbeat returns the control plane's actual decision and the lease window in the database's own clock; the runner converts that window to a **monotonic** budget with a derived safety margin. A definitive refusal — cancellation, fencing, an expired lease — stops the running sandbox promptly; an unreachable control plane consumes the remaining budget and then stops it fail-closed. A transient outage inside the budget does not end a healthy run. Nothing is submitted after authority ends.
- **Execution fencing is distinct from state fencing.** The database refuses a stale worker's writes; the runner stops its computation. Both are required, and a test proving stale results are rejected proves nothing about workload termination.
- The dispatch consumer ships **disabled by default**. It is complete and proven end to end against a real broker. The original reason — that nothing in the repository could send a heartbeat, so enabling it without a worker claimed every run and lost the lease a minute later — no longer applies: a worker renews continuously under ADR-029. It stays disabled because a deployment that enables it without actually running a worker still terminalizes every run as a lost lease, which is a diagnosis no worker earned. Starting the consumer with lease reconciliation disabled is refused outright, because claimed runs would then hold admission capacity with nothing able to release them.
...
- The runner cannot execute any caller-supplied command. It launches Karate only as a fixed, platform-owned program inside a sandbox — a compile-time entrypoint, a compile-time classpath, and a feature set read from the platform's own manifest; nothing a command carries reaches a command line. It also starts containers: a trusted launcher runs one repository-controlled security probe, selected from a fixed server-side enumeration, under an immutable hardened profile. That component holds Docker daemon access and is the most privileged code in the repository.
...
| Capability | Status |
|---|---|
| Karate execution | **IMPLEMENTED, SECRET-FREE ONLY** — Karate 2.1.2 executes tenant features in `services/karate-engine`, the only module carrying it. A run with any secret binding is refused. No reports, no artifacts, no tenant-chosen flags, versions, images, or classpath |
| FeatureRevision execution | **ABSENT** — source enters a sandbox as inert data on a filesystem that refuses to execute it; nothing executes, parses or interprets it |
...
| Tenant-controlled code execution | **IMPLEMENTED FOR SECRET-FREE RUNS** — ADR-033. Tenant code runs under the hostile-execution model: containment is the boundary and the engine classpath is a security control. Secrets, reports and artifacts remain unadjudicated |
...
- A production secret provider, secret capability issuance, or artifact retention. Karate and execution of tenant content are no longer on this list — ADR-033 authorized both for secret-free runs. Tenant source is now *delivered* under ADR-030 and remains inert: it is hashed and compared, never run. A command is delivered and executed under ADR-024, but what it executes is a platform-owned synthetic workload (`KAAS_SYNTHETIC_V1`): no FeatureRevision source enters a sandbox, no secret is resolved, and the declared engine is `SYNTHETIC` rather than `KARATE` — the runner refuses any engine it does not have.
- OpenTelemetry instrumentation.
- Quality evaluation, outbox retention policy, and durable run event storage beyond the scheduling transition record.
...
Karate has now been added, once, under ADR-032's restrictions and adjudicated by ADR-033: one secret-free
execution path, source from the prepared frozen filesystem only, a pinned engine version and platform adapter,
existing egress policies, a bounded platform-shaped result, and no artifacts or report persistence. Of the
three things that slice had to build and could not inherit, two were built as specified — a determination of
`read()`-of-URL behaviour (it is not a network operation in 2.1.2) and a platform-owned result adapter — and
the third, source-read containment bounded to `/kaas/source`, was **deliberately not built**: measurement
showed a wrapper there would not be a boundary while `Files.readAllBytes` is one `Java.type` away, so the
platform confines what there is to read instead. That reversal is recorded in ADR-033 rather than absorbed.
Do not add a real secret provider. Source is read and hashed inside a sandbox under ADR-030 and now lives on a filesystem that refuses to execute it under ADR-031 — both are delivery decisions and neither is an execution one. The next slice is the execution-readiness adjudication, not an engine: the risks accepted so far were accepted *because* the bytes were inert. Egress is no longer on that list — ADR-026 makes `ALLOWLIST` enforceable — but what it authorizes is still a repository-controlled synthetic workload, and nothing about enforceable egress moves the boundary on tenant code. Do not widen the policy grammar with wildcards, CIDRs, or IP literals, and do not enable IPv6 egress, until the address classifier covers them and is tested against them. A run may now outlive one lease period: the worker renews continuously for every worker-owned phase under ADR-029, and a run that stops being renewable is terminated rather than left to finish.

See `HOSTILE_CONTENT_READINESS_SLICE_REPORT.md` for the ADR-022 adjudication: the five-prerequisite matrix, why the output half of "artifact and output handling" is satisfied and the input half is the next slice's subject, the gVisor residual risk stated as reduction and remainder, why `NO_SETUID_BINARIES` plus an empty capability bounding set jointly cover the unobservable `NoNewPrivs`, the finding that signed evidence can outlive the runtime it describes for the freshness window, the two untested guards it uncovered, the third harness defect of the project, and the exact requirements binding on source delivery.

See `CONTINUOUS_EXECUTION_AUTHORITY_SLICE_REPORT.md` for the execution-authority slice: why database fencing alone was insufficient, the heartbeat weakness it replaces, the definitive-versus-transient decision model, the database clock domain and the monotonic budget derived from it, the safety margin and where its floor comes from, the sandbox interruption model and the sliced-wait design that failed silently, gVisor termination, the two harness errors that invalidated results before they were caught, the vacuous leak check inherited from the previous slice, the nineteen-mutation ledger and its four survivors, and the residual risks.
