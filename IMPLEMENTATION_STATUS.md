...
| Capability | Status |
|---|---|
| Karate execution | **ABSENT / DISABLED** — no Karate runtime exists in any dependency graph, and no `.feature` file is parsed by anything |
...
| Tenant-controlled code execution | **NOT APPROVED** — ADR-022 stays open; `noexec` is now closed, and it remains blocked on the `nodev` runtime limitation and the runtime-pin attestation gap |
